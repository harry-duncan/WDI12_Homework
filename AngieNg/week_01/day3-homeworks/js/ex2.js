...
- Write a function called guessLetter that will:
	...
	- When it's done iterating, it should log the current guessed letters ('F__') and congratulate the user if 
	  they found a new letter.
...
var lettersOfWord = ['A', 'P', 'P'];
var guessedLetters = ['_', '_', '_'];
var totalWin = 0;
var attemptNo = 6;


var guessLetter = function(inputGuess) {
	var goodGuess = false;
	...
	if (attemptNo === 0) {
		return console.log("You've finished your games attempt and you can't proceed anymore!");
	}

	/* Call function guessWinning to check guess winning process */
	guessWinning(inputGuess);


	//Starts calculating reward amounts from total winning games
	if (totalWin > 0) {
		
		for (var j = 1; j <= totalWin; j++){
			if (totalWin == 1 || j == 1) {
				rewardAmt = Math.random();
				console.log("First Reward Amount = $" +rewardAmt);
			} else {
		...
		deductAmt = rewardAmt/totalWin;
		attemptNo -= 1;	
		console.log("Sorry, you have no game winning.");		
	}
...
var guessWinning = function(inputGuess) {

	for (var i = 0; i < lettersOfWord.length; i++) {
		if (lettersOfWord[i] == inputGuess.toUpperCase()) {		//if inputGuess is match with guessedLetters
			guessedLetters[i] = inputGuess;

			goodGuess = true;
			totalWin = totalWin + 1;
			console.log("Congratulation! You found new letter " + guessedLetters + '\n');
		...
		if (i == (lettersOfWord.length - 1)) {		// End of games checking
			if (guessedLetters.indexOf('_') < 0){
				console.log("Congratulation! You are winning the game.");
			}
			else {
				console.log("But your game is not over yet, please try again. ");
			}
		}
	}	
}
